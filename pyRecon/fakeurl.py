...
def message():
    system('clear')
    print('''{0}                                                                   
    {0}88888888888     88          88     88  88888888888    {1}     88       88  88888888ba   88           
    {0}88             88 88        88    88   88             {1}     88       88  88      "8b  88           
    {0}88            88   88       88   88    88             {1}     88       88  88      ,8P  88           
    {0}88aaaaa      88     88      88 88      88aaaaa        {1}     88       88  88aaaaaa8P'  88           
    {0}88"""""     88888888888     88 88      88"""""        {1}     88       88  88""""88'    88         
    {0}88         88         88    88   88    88             {1}     88       88  88    `8b    88           
    {0}88        88           88   88    88   88             {1}     8a      a8P  88     `8b   88           
    {0}88       88             88  88     88  88888888888    {1}      "Y8888Y"    88      `8b  88888888  {1}
    ________________________________________________________________________________________________________________

How to use:

 Insert name: example
 Insert level domain: .com'''.format(YELLOW, RED))


def makefake(char, unicd, uninum, newurl, end):
...
def main():
    ...
    if "A" in url.upper():
        makefake('a', names[0], unicodes[0], url.replace('a', '\u0430'), end)
    ...
    if "C" in url.upper():
        makefake('c', names[1], unicodes[1], url.replace('c', '\u03F2'), end)
    ...
    if "E" in url.upper():
        makefake('e', names[2], unicodes[2], url.replace('e', '\u0435'), end)
    ...
    if "O" in url.upper():
        makefake('o', names[3], unicodes[3], url.replace('o', '\u043E'), end)
    ...
    if "P" in url.upper():
        makefake('p', names[4], unicodes[4], url.replace('p', '\u0440'), end)
    ...
    if "S" in url.upper():
        makefake('s', names[5], unicodes[5], url.replace('s', '\u0455'), end)
    ...
    if "D" in url.upper():
        makefake('d', names[6], unicodes[6], url.replace('d', '\u0501'), end)
    ...
    if "Q" in url.upper():
        makefake('q', names[7], unicodes[7], url.replace('q', '\u051B'), end)
    ...
    if "W" in url.upper():
        makefake('w', names[8], unicodes[8], url.replace('w','\u051D'), end)
        urlMore = urlMore.replace('w', '\u051D')
        urlChars += 'w.'
        urlNms += names[8] + '.'
        urlUn += unicodes[8] + '.'

    print ('\n\n{0}[   MORE EXTENSIVE fake URL:   ]{1}'.format(RED, END))
    makefake(urlChars, urlNms, urlUn, urlMore, end)
...
def check_fake(url):
    ...
    '''
    Check fake chars in URL
    :param url: suspicious URL
    :return: result of check and the fake chars
    '''
    ...
    if result:
        msg = '\n{0}[*] fake URL detected: {1}{2}{3}{1}'.format(YELLOW,END,RED,url)
        msg += '\n{0}[*] fake characters used: {1}{2}{3}{1}'.format(YELLOW,END,RED,result)
    else:
        msg = '\n{0}[*] fake URL NOT detected:{1} {2}{3}{1}'.format(YELLOW, END, GREEN, url)
...
def urls_list(file):
    '''
    Read the file to verify fake URL
    :param file: file with a list of fake URLs 
    :return: file reading
    '''

    with open(file) as arq:
        urls = [f.strip() for f in arq]
    for i in range(len(urls)): print(check_fake(urls[i]))


def check_list_url(file):

    '''
    Check fake chars in list of suspicious fake URL
    :param file: file with a list of fake URLs
    ...
    for url in urls_arq:
        ...
        if result:
            msg += '\n{0}[*] fake URL detected: {1}{2}{3}{1}'.format(YELLOW, END, RED, url)
            msg += '\n{0}[*] fake characters used: {1}{2}{3}{1}\n'.format(YELLOW, END, RED, result)
            msg += check_result

        else:
            msg += '\n{0}[*] fake URL NOT detected:{1} {2}{3}{1}\n'.format(YELLOW, END, GREEN, url)
            msg += check_result
...
def Runner():
    message()
    print ('\nSelect an option: \n\n{0}[{1}1{0}]{1} Generate fake urls\n{0}[{1}2{0}]{1} Detect fake urls'.format(RED, END))
    opt = input('\n>{0}>{1}> '.format(RED, END))
    if opt == '1':
        main()
    elif opt == '2':
